...
// VersionCompare
//  @Description The possible operators are: <, lt, <=, le, >, gt, >=, ge, ==, =, eq, !=, <>, ne respectively.
...
func VersionCompare(version1, version2, operator string) (bool, error) {
	var compareHandler func(string, string) int
	var canonicalize func(string) string
	var special func(string, string) int

	// version compare
	compareHandler = func(origV1, origV2 string) int {
		if origV1 == "" || origV2 == "" {
			if origV1 == "" && origV2 == "" {
				return 0
			}
		...
		if compare == 0 {
			if ver1 != "" {
				if ver1[0] >= '0' && ver1[0] <= '9' {
					...
					compare = compareHandler(ver1, "#N#")
				}
			} else if ver2 != "" {
				if ver2[0] >= '0' && ver2[0] <= '9' {
					compare = -1
				} else {
					compare = compareHandler("#N#", ver2)
				}
			}
		}
	...
	compare := compareHandler(version1, version2)
	...
	case "!=", "<>", "ne":
		return compare != 0, nil
	default:
		return false, errors.New("operator: invalid")
	}
}
