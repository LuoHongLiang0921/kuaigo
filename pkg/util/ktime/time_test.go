...
func Test_Time(t *testing.T) {
	...
	parse :=Parse("2020-08-05").ToDateTimeString()
	tomorrow :=Now().Tomorrow().Format(DateTimeFormat)
	Yesterday :=Now().Yesterday().Format(DateFormat)
...
func Test_Format(t *testing.T) {
	...
	now3 := Now().Format("Y-m-d")
	now4 := Now().Format("Y/m/d")
	fmt.Printf("Now : %+v\n", now)
	fmt.Printf("now2 : %+v\n", now2)
	fmt.Printf("now3 : %+v\n", now3)
	fmt.Printf("now4 : %+v\n", now4)
