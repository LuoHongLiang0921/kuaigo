...
func (s *Server) Info() *kserver.ServiceInfo {
	serviceAddr := s.listener.Addr().String()
	if s.config.ServiceAddress != "" {
		serviceAddr = s.config.ServiceAddress
	}

	info := kserver.ApplyOptions(
		kserver.WithScheme("http"),
		kserver.WithAddress(serviceAddr),
		kserver.WithKind(constant.ServiceProvider),
	)
