...
func defaultServiceInfo() ServiceInfo {
	si := ServiceInfo{
		...
		Region:     kpkg.AppRegion(),
		Zone:       kpkg.AppZone(),
		Kind:       0,
		Deployment: "",
		Group:      "",
	}
	si.Metadata["appMode"] = kpkg.AppMode()
	si.Metadata["appHost"] = kpkg.AppHost()
	si.Metadata["startTime"] = kpkg.StartTime()
	si.Metadata["buildTime"] = kpkg.BuildTime()
	si.Metadata["appVersion"] = kpkg.AppVersion()
	si.Metadata["tabbyVersion"] = kpkg.TabbyVersion()
	return si
}
